#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────
# TINC Köle — Router Guardian install.sh
# ──────────────────────────────────────────────────────────────
set -euo pipefail
...
if [ ! -f "/etc/kole/config.env" ]; then
    ...
    mkdir -p /etc/kole
    cat > /etc/kole/config.env <<'CONFIG'
...
CONFIG
    echo "   ✏️  /etc/kole/config.env oluşturuldu. Lütfen değerleri düzenleyin."
fi
